...
- **Notification** — the in-app notification inbox (a plain `notifications` table and model of our own; Laravel's built-in `Notifiable`/`Illuminate\Notifications` system is deliberately *not* used, and adding `Notifiable` to `User` would clash with `User::notifications()`). Routes: `GET /api/v1/notifications` (paginated, `per_page` default 20 max 100, optional `status=read|unread`), `PATCH /notifications/{id}/read` and `DELETE /notifications/{id}` (soft delete); same `findFor`/`findOwned` 404 rule as Ledger. Each row has `title`, `message`, `type` (`NotificationType`: info/success/warning/error), `subject` (an event key such as `category_limit_warning`, `installment_due_soon`, `installment_overdue`, `goal_deadline`) and a nullable `subject_id` pointing at the related record (no FK, since it can reference different tables). Other domains create notifications only through `NotificationService::notify()` or `notifyOnce()`; `notifyOnce()` skips creation when a row with the same `subject` + `subject_id` (optionally since a date) already exists, and that check uses `withTrashed()` on purpose so a notification the user deleted never comes back — which is why notifications keep `SoftDeletes`. Producers: `CategoryService::warnIfNearLimit()` (called by `TransactionService` after an expense is saved; once per category per month at ≥ 80% of `limit_amount`, while `ensureInMonthLimit()` still blocks anything over 100%) and `Ledger\Services\ReminderService` (installments due within 2 days, overdue installments, unreached goals expiring within 7 days; each notified once), run daily at 08:00 America/Sao_Paulo by the `ledger:send-reminders` command in `routes/console.php`. The scheduler only fires while `php artisan schedule:work` (dev) or a `schedule:run` cron is running; the Docker setup does not start one. `DeviceToken` (FCM push tokens, `DevicePlatform` enum, repository only) is groundwork for push notifications and is not wired to any route yet.
...
Cross-slice references are allowed but should stay rare and explicit. Today: `Identity\Models\User::goals()` and `User::categories()` point at `Ledger\Models\Goal` / `Category`, `User::notifications()` / `User::deviceTokens()` point at the Notification models, Ledger depends on Shared, and Ledger services call `NotificationService` (never its repository) to create notifications.
